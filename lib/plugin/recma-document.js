  ...
  return function (tree, file) {
      ...
      walk(tree, {
        enter(node) {
          if (
            ...
            if(!baseHref){ throw new Error('unexpected missing `baseHref` in branch'); }

            let value = node.source.value
            // The literal source for statements can only be string.
            if (typeof value !== 'string'){ throw new Error('expected string source'); }
    ...
    if (usesImportMetaUrlVariable) {
      if (
        ...
        throw new Error('expected `function-body` when using dynamic url injection');
      }
    ...
    function handleEsm(node) {
      ...
      if (outputFormat === 'function-body') {
        if (
          ...
          if (!node.source) { throw new Error('expected `node.source` to be defined'); }
